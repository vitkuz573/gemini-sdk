...
pub enum CredentialsError {
    ...
    MissingPsidcc,
    /// Multiple mutually inconsistent PAPISID-style cookies are present.
    InconsistentSapisid,
}
...
impl fmt::Display for CredentialsError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InconsistentSapisid => {
                write!(f, "inconsistent `{PAPISID}` / `{SAPISID}` / `{APISID}` cookies")
            }
        }
    }
}
...
impl Credentials {
    ...
    pub fn validate(&self) -> Result<(), CredentialsError> {
        ...
        let sapisid_values: Vec<&str> = [self.papisid.as_deref(), self.sapisid.as_deref(), self.apisid.as_deref()]
            .into_iter()
            .flatten()
            .collect();
        let first = sapisid_values.first().copied();
        if let Some(first) = first {
            if sapisid_values.iter().any(|v| *v != first) {
                return Err(CredentialsError::InconsistentSapisid);
            }
        }
        Ok(())
    }

    /// Returns the value that should be used for SAPISIDHASH-style
    /// `Authorization` headers.
    ///
    /// This prefers `__Secure-1PAPISID`, then `SAPISID`, then `APISID`.
    #[must_use]
    pub fn sapisid_value(&self) -> Option<&str> {
        self.papisid
            .as_deref()
            .or(self.sapisid.as_deref())
            .or(self.apisid.as_deref())
    }

    /// Builds the `Authorization: SAPISIDHASH ...` header value if a SAPISID
    /// cookie is available.
    ///
    /// The hash format mirrors the one generated by Google's web clients:
    /// `SAPISIDHASH <unix_seconds_hex>_<sha1_of_origin + sapisid>`.
    #[must_use]
    pub fn authorization_header(&self) -> Option<String> {
        ...
        let now = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        let input = format!("{origin}{sapisid}");
        let hash = sha1_smol::Sha1::from(input.as_bytes()).hexdigest();
        Some(format!("SAPISIDHASH {:x}_{}", now, hash))
    }
...
mod tests {
    ...
    #[test]
    fn credentials_detect_inconsistent_sapisid() {
        let mut creds = Credentials::new();
        creds.psid = "x".to_string();
        creds.psidcc = "y".to_string();
        creds.papisid = Some("a".to_string());
        creds.sapisid = Some("b".to_string());
        assert_eq!(creds.validate().unwrap_err(), CredentialsError::InconsistentSapisid);
    }

    #[test]
    fn credentials_sapisid_value_prefers_papisid() {
        let mut creds = Credentials::new();
        creds.psid = "x".to_string();
        creds.psidcc = "y".to_string();
        creds.papisid = Some("papi".to_string());
        creds.sapisid = Some("sapi".to_string());
        assert_eq!(creds.sapisid_value(), Some("papi"));
    }

    #[test]
    fn credentials_authorization_header_has_expected_format() {
        let header = format!("{PSID}=x; {PSIDCC}=y; {PAPISID}=sapisid_value");
        let creds = Credentials::from_header(&header).unwrap();
        let auth = creds.authorization_header().expect("expected auth header");
        assert!(auth.starts_with("SAPISIDHASH "));
        let rest = &auth["SAPISIDHASH ".len()..];
        let (timestamp, hash) = rest.split_once('_').expect("expected underscore separator");
