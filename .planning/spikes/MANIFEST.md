...
| 008 | cookie-auth | standard | Determine why browser cookies copied into the SDK produce unsigned `/app` HTML and identify the missing cookies/headers | ✓ VALIDATED | gemini, auth, cookies, signed-in, har, SID, SSID, legacy-cookies |
| 009 | har-api-coverage | standard | Given the 135 MB HAR capture at ~/mitm.har, compare observed undocumented API traffic with the current SDK and report coverage, gaps, and drift | ✓ VALIDATED | gemini, har, api-coverage, reverse-engineering, batchexecute, stream-generate, waa, ogads |
