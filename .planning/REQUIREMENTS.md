...
- [ ] **GEN-02**: Keep the generator behind a new optional Cargo feature (`browserless-waa`).
...
- [ ] **GEN-04**: Choose the implementation backend based on what actually reproduces captured tokens: pure Rust algorithm if deterministic, V8/Chromium/Playwright/QuickJS harness if a real browser engine is required. Do not reject heavier dependencies just to keep the crate small.
...
| Embedded V8 / Deno / Node runtime | Heavy, but acceptable if they are the only way to run the BotGuard VM correctly. Prefer isolated processes or feature-gated crates over forcing a full browser when a JS engine suffices. |
